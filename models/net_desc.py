        ...
        for decoder_name, output_head in self.decoder_info_list.items():
            ...
            if decoder_name in self.considered_tasks:
                if decoder_name == "Patch-Class":
                    ...
                    # input to the patch-classification head is the bottom
                    # feature map BEFORE conv_map (see forward), i.e. filters[-1]
                    bottom_ch = filters[-1]
                    for output_name, output_ch in output_head.items():
                        module_list = [
                            ("bn1", nn.BatchNorm2d(bottom_ch, eps=1e-5)),
                            ("relu1", nn.ReLU(inplace=True)),
                            ("dropout", nn.Dropout(p=0.3)),
                            ("conv1", nn.Conv2d(bottom_ch, 256, 1, stride=1, padding=0)),
